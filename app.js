    ...
    message to the webhook. */
        for(let i = 0; i < generalID.length; i++) {
            if (messageCreate.channel.id === generalID[i]) {
                ...
                /* Trying to send the message to the webhook. If it fails, it will send the error
                message to the webhookErr. */
                try{
                    if (typeof content1 !== 'string') throw new error(errorMessage);
                    webhooks[i].send({ content: content1, username: username2, avatarURL: avatar2 });
                }catch(error){
                    let errString = JSON.stringify(content1);
                    console.log(JSON.stringify(content1));
                    webhookErr.send({ content: errString, username: username2, avatarURL: avatar2 });
                }
            }
        }
});
